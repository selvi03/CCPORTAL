...
import { getStudents_Course_LMS_API,getSkilltypeApi,getcontenttoolApi } from "../../api/endpoints";
...
const LearningMaterial = ({ username,institute }) => {
  ...
  const [selectedFeedbackSource, setSelectedFeedbackSource] = useState(null);
...
// Handle page change
 ...
 const getLMSContents = () => {
  getStudents_Course_LMS_API(username, selectedQuestionType, selectedSkillType)
    .then((data) => {
...
const handleOpenFeedback = (id) => {
  ...
  const selectedContent = testcontents.find(item => item.id === id);

  if (selectedContent) {
    setSelectedFeedbackId(id);
    setSelectedFeedbackSource(selectedContent.source); // ✅ safe access
    setShowFeedbackModal(true);
  } else {
    console.warn("Content not found for the given ID:", id);
  }
};
  ...
  return (
    <div className="no-select no-right-click">
      ...
      <div
        className="product-table-container-student"
                      ...
                      <td>
                        {content.Content_URL ? (
                          ...
                          >
                            ...
                            <span style={{ color: "white" }}> Play Video</span>
                          </Button>
                        ) : (
                          <Button
                            variant="link"
