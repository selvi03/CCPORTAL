...
const InvoiceTB = (username) => {
  ...
  useEffect(() => {
    getInvoices();
  }, [username]); // Fetch invoices when the username changes
 
  const getInvoices = () => {
    console.log("Fetching invoices for username:", username);
  
    Get_Invoice_API(username)
      .then((response) => {
        ...
        if (allInvoices.length === 0) {
          console.warn(`No invoices found for username "${username}".`);
          setErrorMessage("No invoices found. Try again later.");
          setShowError(true);
          return;
        }
  
        // Filter invoices where trainer_id__user_name matches username
        const filteredInvoices = allInvoices.filter((item) => {
          let trainerName = ""; // Default value
  
          // Safely extract the trainer_id__user_name value
          if (typeof item.trainer_id__user_name === "object" && item.trainer_id__user_name !== null) {
            // Extract from nested object if present
            trainerName = item.trainer_id__user_name.name || item.trainer_id__user_name.username || "";
          } else if (typeof item.trainer_id__user_name === "string") {
            // Direct string access
            trainerName = item.trainer_id__user_name;
          }
  
          console.log(`Comparing: "${trainerName}" with "${username}"`);
  
          // Compare safely
          return trainerName === username.username; // Assuming username is an object {username: "Asha"}
        });
  
        console.log("Filtered Invoices:", filteredInvoices);
  
        // Set filtered or fallback invoices
        if (filteredInvoices.length > 0) {
          setInvoices(filteredInvoices);
        } else {
          console.warn(`No matching invoices found for username "${username.username}".`);
          setErrorMessage("No matching invoices found. Showing all invoices.");
          setInvoices(allInvoices); // Show all invoices
        }
      })
      .catch((error) => {
        console.error("Error fetching invoices:", error);
        setErrorMessage("Failed to fetch invoices. Try again.");
        setShowError(true);
      });
  };
